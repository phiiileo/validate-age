...
const calaculateAge = (dob) => {
    ...
    if (dobformated == "Invalid Date") {
        ...
        if (dobformated == "Invalid Date") throw new ValidationError('Please pass a valid date to continue');
    }
    // get the time difference
    const timedifference = Date.now() - dobformated;
    
    if (timedifference < 0) throw new ValidationError(`Your date of birth cannot be greater than the current year`);
